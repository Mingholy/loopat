 ...
 *      CONTAINED vault (fresh ed25519 + IDEALAB_API_KEY from env) pointing at
...
async function bringUpServer(opts: {
  ...
  // self-contained onboarded vault: fresh ssh keypair + IDEALAB_API_KEY
  ...
  writeFileSync(join(envsDir, "IDEALAB_API_KEY"), (process.env.IDEALAB_API_KEY ?? "") + "\n");
  ...
  // personal config: idealab provider + shared kn/notes/roster pointers
